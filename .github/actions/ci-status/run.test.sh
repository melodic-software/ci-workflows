...
# run_case <expected-status> <results> <treat-skipped-as> <event-action>
run_case() {
  local expected_status="$1" results="$2" treat_skipped_as="$3" event_action="$4"
  local actual_status
  : >"$gh_log"
  rm -rf -- "$calls"
  mkdir -p "$calls"
  ...
  env \
    ...
    CARRY_FORWARD_ACTIONS='edited,labeled,unlabeled' \
...
# Without the carry-forward membership test this would read a status instead of
# aggregating, and no fixture status exists for it.
echo 'case: an event action outside the carry-forward list aggregates normally'
run_case 1 'success failure' pass synchronize
expect_log 'A lane did not pass (result: failure).'
expect_no_gh_call "commits/${sha}/status"

# Without the empty-EVENT_ACTION guard a push run would take the carry-forward
# branch on the empty string.
echo 'case: an empty event action (push) aggregates normally'
run_case 0 'success success' pass ''
expect_log 'All lanes passed or were skipped.'
expect_no_gh_call "commits/${sha}/status"
...
run_case 0 'skipped skipped' fail edited
...
run_case 1 'skipped skipped' pass labeled
...
run_case 1 'skipped skipped' pass unlabeled
expect_log "::error::no successful ci-lanes status on ${sha}; re-run the full workflow"
...
run_case 1 'skipped skipped' pass edited
expect_log "::error::no successful ci-lanes status on ${sha}; re-run the full workflow"
...
run_case 0 'skipped skipped' pass labeled
expect_log "Carried forward: ci-lanes is success on ${sha}."
...
for input_name in results treat-skipped-as event-action carry-forward-actions status-context token repository sha; do
  if ! grep -qE "^  ${input_name}:" "$action_metadata"; then
    echo "FAIL: action.yml declares no '${input_name}' input"
    failures=$((failures + 1))
  fi
done
for environment_name in RESULTS TREAT_SKIPPED_AS EVENT_ACTION CARRY_FORWARD_ACTIONS STATUS_CONTEXT GH_TOKEN REPOSITORY SHA; do
