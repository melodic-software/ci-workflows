...
function selectIdleCandidate(runners, labels, managedRunnerPrefix) {
  ...
  // runner carrying it. Treat that exact label as contaminated if even one
  // bearer falls outside the managed namespace or explicitly reports itself
  ...
  const contaminatedLabels = new Set();
  for (const runner of runners) {
    if (
      ...
      for (const runnerLabel of runner.labels) {
        if (labels.includes(runnerLabel.name)) {
          contaminatedLabels.add(runnerLabel.name);
        }
      }
    }
  }
  const safeLabels = labels.filter((label) => !contaminatedLabels.has(label));
  ...
  const idleRunners = runners.filter((runner) => {
    ...
    return (
      ...
      runner.labels.some((runnerLabel) => safeLabels.includes(runnerLabel.name))
    );
  });

  const selectedLabel = safeLabels.find((label) =>
  ...
  return {
    ...
    labelNamespaceInvalid:
      contaminatedLabels.size > 0 && safeLabels.length === 0,
  };
}
