...
function renderIssueBody(state) {
  ...
  const prologue = [
    ...
    `- Consecutive clean cycles: ${state.cleanCycles} of ${CLEAN_CYCLES_TO_CLOSE} (auto-closes at ${CLEAN_CYCLES_TO_CLOSE}; only cycles that polled every repository this incident tracks count)`,
  ...
  const epilogue = [
    ...
    "4. A clean cycle only counts if it polled every repository this incident",
    "   tracks. That is the durable index behind this report, not the table",
    "   above: the table is capped separately, so a fleet-wide incident tracks",
    "   more repositories than it lists. Polling scope is the aggregator App's",
    "   installation, so a missing App credential narrows it to this repository",
    "   and stops the count without failing anything; the run log's `coverage=`",
    "   field and its warning annotation name what went unobserved. Three",
    "   causes hold an incident open for good, and each is closed by hand once",
    "   you have confirmed recovery: a repository that is gone — archived, or",
    "   removed from the installation — can never be covered again; an incident",
    "   wider than the tracked index can never account for what the index",
    "   dropped; and a state-schema bump strands every incident open at the",
    "   time.",
