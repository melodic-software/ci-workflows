...
function renderIssueBody(state) {
  ...
  const epilogue = [
    ...
    "   lists, and can have seen more still. An index naming nothing satisfies",
    "   the last two conditions by vacuum and still counts no cycle, which is",
    "   why the first one is stated at all.",
    ...
    "   open for good, and each is closed by hand once the failure it names is",
    "   resolved or no longer applies — two of them leave nothing to recover:",
