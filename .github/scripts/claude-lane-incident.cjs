...
function renderIssueBody(state) {
  ...
  const epilogue = [
    ...
    "   tracks — which can be more than the table above shows. Polling scope",
    "   is the aggregator App's installation, so a missing App credential",
    "   narrows it to this repository and stops the count without failing",
    "   anything; the run log's `coverage=` field and its warning annotation",
    "   name what went unobserved. Coverage becomes unprovable for good once",
    "   a tracked repository is archived or removed from the installation,",
    "   once an incident grows wider than this watchdog tracks, or across a",
    "   change to the state format stored below — in each case close this by",
    "   hand after confirming recovery.",
