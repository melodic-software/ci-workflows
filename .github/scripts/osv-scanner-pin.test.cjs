...
const pin = JSON.parse(fs.readFileSync(path.join(repositoryRoot, ".github", "osv-scanner-pin.json"), "utf8"));
const workflow = fs.readFileSync(path.join(repositoryRoot, ".github", "workflows", "osv-scanner.yml"), "utf8");
const driftWorkflow = fs.readFileSync(path.join(repositoryRoot, ".github", "workflows", "tool-version-drift-check.yml"), "utf8");
...
test("workflow runs the native binary on a caller-selected runner and verifies SLSA", () => {
  ...
  assert.match(workflow, new RegExp(`OSV_PROVENANCE_SHA256: ${pin.provenance.sha256}`, "u"));
  assert.match(workflow, new RegExp(`SLSA_VERIFIER_VERSION: ${pin.verifier.version}`, "u"));
  assert.match(workflow, new RegExp(`SLSA_VERIFIER_SHA256: ${pin.verifier.sha256}`, "u"));
...
test("OSV result handling is generated from the tested fail-closed guard", () => {
  ...
  assert.equal(workflow.match(/Source: \.github\/scripts\/osv-scan-guard\.sh/gu)?.length, 1);
  assert.match(workflow, /operationally \(exit \$SCAN_EXIT\); results are not trusted/u);
  assert.match(workflow, /exit and result mismatch/u.test(workflow) ? /exit and result mismatch/u : /disagrees with SARIF finding count/u);
