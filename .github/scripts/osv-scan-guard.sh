...
annotate_findings() {
  ...
  # A workflow-command PROPERTY value escapes everything a DATA value does and
  # two delimiters besides, so it is exactly `escape_data` plus those two. The
  # order matters and is preserved: `%` is encoded first, so the `%` each later
  # substitution introduces is left alone. `escape_data` emits no newline and
  # can produce none (both CR and LF are already encoded), so the command
  # substitution has nothing to strip.
  escape_property() {
    local v
    v="$(escape_data "$1")"
    v=${v//':'/'%3A'}
