...
function inputDefault(actionName, inputName) {
  const source = read(path.join(".github", "actions", actionName, "action.yml"));
...
test("Python action defaults are the same exact patch release", () => {
  ...
  const versions = actionNames.map((name) => inputDefault(name, "python-version"));
...
test("drift workflow protects runtime and schema agreement", () => {
  ...
  assert.match(
    workflow,
    /markdown_schema_version" != "\$markdown_current"/u,
  );
