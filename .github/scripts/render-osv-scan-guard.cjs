...
const workflowPath = path.join(scriptsDirectory, "..", "workflows", "osv-scanner.yml");
...
function render(workflow, source) {
  ...
  if (start < 0 || end < start) throw new Error("osv-scanner.yml is missing generated guard markers");
...
if (process.argv.includes("--check")) {
  if (current !== expected) {
    process.stderr.write("osv-scanner.yml is out of sync; run node .github/scripts/render-osv-scan-guard.cjs\n");
    process.exitCode = 1;
  }
} else {
